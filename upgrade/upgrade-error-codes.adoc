...
| 084 | Echec de la remise en production du JobManager | Pour récupérer/terminer la mise à niveau, procédez comme suit : 1. Activez l’interface de diagnostic 2. Exécutez la commande : _sudo perl /home/maint/scripts/post-deploy-upgrade.pl --postupgrade_ 3. Consultez les journaux dans /var/log/post-deploy-upgrade.log 
...
| 090 | la validation du certificat pour les backends de stockage et vCenter a échoué | Vérifiez les journaux de mise à niveau et le fichier journal dans /var/log/cert_validation_error.log et réessayez la mise à niveau. 
