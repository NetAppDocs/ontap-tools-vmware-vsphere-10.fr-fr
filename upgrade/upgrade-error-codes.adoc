...
summary: 'Utilisez les codes d"erreur de mise à niveau des outils ONTAP pour identifier les flux de travail ayant échoué et résoudre les problèmes de mise à niveau dans VMware vSphere.' 
---
= Codes d'erreur de mise à niveau des ONTAP tools
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
| 068 | La restauration des paquets du système d'exploitation Debian a échoué | Utilisez une restauration basée sur des snapshots ou un RPO nul et réessayez la mise à niveau. 
...
| 074 | Echec du montage ISO | Vérifiez le fichier /var/log/otvng/upgrade-run.log et réessayez la mise à niveau. 
...
| 084 | Echec de la remise en production du JobManager | Pour récupérer/terminer la mise à niveau, procédez comme suit : 1. Activez l’interface de diagnostic 2. Exécutez la commande : _sudo perl /home/maint/scripts/post-deploy-upgrade.pl --postupgrade_ 3. Consultez les journaux dans /var/log/otvng/post-deploy-upgrade.log 


| 087 | Échec des étapes post-mise à niveau. | Pour récupérer/terminer la mise à niveau, effectuez les étapes suivantes. 1. Activez l’interface de diagnostic 2. Exécutez la commande _sudo perl /home/maint/scripts/post-deploy-upgrade.pl --postupgrade_ 3. Vérifiez les journaux à l’emplacement /var/log/otvng/post-deploy-upgrade.log 


| 088 | La configuration de la rotation du journal pour journald a échoué | Vérifiez que les paramètres réseau de la machine virtuelle sont compatibles avec l'hôte qui l'héberge. Vous pouvez essayer de migrer la machine virtuelle vers un autre hôte et de la redémarrer. 
...
| 098 | La copie du secret dockercred de l'espace de noms ntv-system vers l'espace de noms rook-ceph a échoué | Consultez les journaux de mise à niveau et réessayez la mise à niveau. 
...
| 090 | la validation du certificat pour les backends de stockage et vCenter a échoué | Vérifiez les journaux de mise à niveau et le fichier journal à /var/log/otvng/cert_validation_error.log et réessayez la mise à niveau. 


| 114 | La mise à jour du certificat interne dans la collection MongoDB a échoué  a| 
Si la mise à niveau échoue avec le code d'erreur 114, procédez comme suit :

. Connectez-vous à l'ISO de mise à niveau sur le nœud principal et activez le shell de diagnostic.
. Monter l'ISO depuis la session SSH sur le nœud principal :
+
[listing]
----
...
----
. Copiez le fichier d'informations de compilation :
+
[listing]
----
sudo cp /upgrade/APPLIANCE_BUILD /
----
. Ajoutez les certificats à la collection en activant l'interface de diagnostic et en exécutant :
+
[listing]
----
...
----
. Effectuez les étapes suivant la mise à niveau :
+
[listing]
----
sudo perl /home/maint/scripts/post-deploy-upgrade.pl --postupgrade
----
. Vérifiez que l'opération a bien été effectuée en consultant les journaux :
+
[listing]
----
...
----
+
Recherchez le message de réussite : `Completed post 10.5 upgrade steps.`
