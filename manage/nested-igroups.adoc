...
[role="lead"]
Si vous gérez à la fois les machines virtuelles ONTAP tools et les systèmes de stockage ONTAP, il est essentiel de comprendre le comportement des igroups, notamment lors du déplacement de datastores d'environnements non gérés par ONTAP tools vers des environnements qui le sont. ONTAP tools pour VMware vSphere représente et gère les igroups à l'aide d'un modèle d'igroup imbriqué, et comprendre ce modèle vous aide à interpréter les différences de comportement entre les versions précédentes et l'implémentation actuelle.
