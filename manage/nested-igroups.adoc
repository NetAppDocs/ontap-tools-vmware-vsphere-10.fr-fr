...
À partir de ONTAP tools for VMware vSphere 10.4, les outils ONTAP créent et gèrent automatiquement les objets ONTAP et vCenter, simplifiant la gestion des datastores dans les environnements de datacenter VMware.
...
NOTE: Seuls les igroups personnalisés peuvent être réutilisés. Les igroups créés lors de la migration depuis ONTAP tools 9.x (igroups migrés) ne peuvent pas être réutilisés et, à partir de ONTAP tools 10.5P2, sont automatiquement supprimés lorsqu'ils ne sont plus utilisés.
