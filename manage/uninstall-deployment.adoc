...
summary: Terminez correctement le déploiement de vos outils ONTAP pour VMware vSphere. 
---
= Désinstallez votre déploiement ONTAP tools for VMware vSphere
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
La désinstallation des ONTAP tools for VMware vSphere supprime les données de l'appliance. Suivez donc ces étapes pour arrêter le déploiement correctement et éviter toute perte de données.
...
** Pour les déplacer vers un datastore non géré, reportez-vous à  https://techdocs.broadcom.com/["Broadcom Documentation technique"] pour la migration des machines virtuelles avec Storage vMotion


. link:../manage/delete-ds.html["Supprimer les datastores"] Créé sur les outils ONTAP pour VMware vSphere.
. Si vous avez activé le VASA Provider, sélectionnez *Paramètres* > *Paramètres du VASA Provider* > *Désinscrire* dans ONTAP tools pour désinscrire le VASA Provider de toutes les instances du serveur vCenter.
