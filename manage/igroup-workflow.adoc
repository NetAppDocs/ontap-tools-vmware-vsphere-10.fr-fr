...
summary: 'Comprenez comment ONTAP tools crée et gère les igroups et les export policies pour l"accès aux datastores.' 
---
= Workflows de groupe initiateur (Igroup) et de règles d'export dans les outils ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Utilisez ces informations pour comprendre comment ONTAP tools for VMware vSphere crée et gère les igroups et les règles d'export pour l'accès aux datastores.

Dans ONTAP tools for VMware vSphere 9.x, les igroups étaient créés et gérés selon une structure plate, où chaque datastore dans vCenter était associé à un seul igroup. Ce modèle limitait la flexibilité et la réutilisation des igroups entre plusieurs datastores. ONTAP tools for VMware vSphere 10.4 introduit les igroups imbriqués, où chaque datastore dans vCenter est associé à un igroup parent, tandis que chaque hôte est lié à un igroup enfant sous ce parent. Vous pouvez définir des igroups parents personnalisés avec des noms définis par l'utilisateur pour une réutilisation entre les datastores afin de faciliter la gestion des igroups. Utilisez ces informations pour comprendre comment ONTAP tools for VMware vSphere gère les LUN et les datastores via les workflows d'igroup. Différents workflows génèrent des configurations d'igroup différentes, comme illustré dans les exemples suivants :


NOTE: Les noms mentionnés sont uniquement à des fins d'illustration et ne font pas référence aux vrais noms de groupes i.  Les groupes gérés par les outils ONTAP utilisent le préfixe « otv_ ».  Les groupes i personnalisés peuvent recevoir n'importe quel nom.
...
ONTAP tools for VMware vSphere crée un igroup enfant host2Igroup et l’ajoute à l’igroup parent existant DS1Igroup.
...
Lorsque vous supprimez ou démontez une banque de données NFS d'un hôte ESXi, ONTAP tools for VMware vSphere supprime l'adresse IP de l'hôte de la règles d'export. Si aucun autre hôte n'utilise cette règles d'export, elle sera supprimée. Lorsque vous supprimez une banque de données NFS, ONTAP tools for VMware vSphere supprime la règles d'export associée à cette banque de données si elle n'est pas réutilisée par d'autres banques de données. Si la règles d'export est réutilisée, elle conserve l'adresse IP de l'hôte et ne change pas. Lorsque vous supprimez des banques de données, ONTAP tools supprime l'adresse IP de l'hôte de la règles d'export et attribue la règles d'export par défaut si ONTAP requiert un accès.
