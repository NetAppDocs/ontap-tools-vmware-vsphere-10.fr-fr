...
[role="lead"]
Gérez la journalisation d'audit dans ONTAP tools for VMware vSphere. Les journaux d'audit indiquent qui a effectué une action, ce qui s'est passé, quand cela s'est produit, la source de la requête et si l'action a réussi ou échoué.
