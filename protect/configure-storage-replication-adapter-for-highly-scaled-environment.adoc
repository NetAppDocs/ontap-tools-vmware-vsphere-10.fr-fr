...
summary: 'Configurez les paramètres de délai d"expiration du fournisseur de stockage VMware Live Site Recovery pour SRA dans les environnements à forte évolutivité.' 
---
= Paramètres de délai d'expiration SRA pour les environnements à grande échelle dans les outils ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Le délai d'attente spécifié correspond à la valeur maximale. Il n'est pas nécessaire d'attendre que le délai d'attente maximal soit atteint. La plupart des commandes s'exécutent dans l'intervalle de délai d'attente maximal défini.
