...
Lors de l'enregistrement d'une instance de serveur vCenter auprès de ONTAP tools Manager, assurez-vous que le certificat du serveur vCenter inclut une extension Subject Alternative Name (SAN) valide. L'extension SAN doit contenir à la fois des entrées DNS et des adresses IP. Par exemple :
...
----
Si le certificat du serveur vCenter n'inclut pas d'extension SAN, ou si l'extension SAN ne contient pas les valeurs DNS ou d'adresse IP correctes, les opérations des ONTAP tools peuvent échouer en raison d'erreurs de validation du certificat.
