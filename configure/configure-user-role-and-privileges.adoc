...
summary: Configurez les rôles et privilèges des utilisateurs pour les backends de stockage avec le fichier JSON des ONTAP tools et ONTAP System Manager. 
---
...
[role="lead"]
Configurez les rôles et privilèges des utilisateurs ONTAP pour les backends de stockage à l'aide des ONTAP tools for VMware vSphere et d'ONTAP System Manager. Vous pouvez attribuer des rôles avec les fichiers JSON fournis, créer manuellement des utilisateurs et des rôles, ou appliquer les privilèges minimaux requis aux comptes non administrateurs.

.Avant de commencer
* Téléchargez le fichier ONTAP Privileges depuis ONTAP tools for VMware vSphere à l'adresse _\https://<ONTAPtoolsIP>:8443/virtualization/user-privileges/users_roles.zip_. Le fichier zip contient deux fichiers JSON. Utilisez le fichier JSON spécifique à ASA r2 lorsque vous configurez un système ASA r2.
+
...
.. Sélectionnez la capacité du produit *VSC, VASA Provider et SRA* dans la liste déroulante.
+
...
.. Saisissez les applications auxquelles vous souhaitez donner accès et la méthode d'authentification. ONTAPI et HTTP sont les applications requises, et le type d'authentification est *Password*.
...
[[list-of-minimum-privileges-required-for-a-non-admin-global-scoped-cluster-user]]
=== Liste des privilèges minimaux requis pour un utilisateur de cluster non administrateur à portée globale

Cette page répertorie les privilèges minimaux requis pour un utilisateur de cluster à portée globale non administrateur, sans fichier JSON. Si un cluster est en portée locale, utilisez le fichier JSON pour créer les utilisateurs, car ONTAP tools for VMware vSphere nécessite plus que les seuls privilèges de lecture pour le provisionnement sur ONTAP.
