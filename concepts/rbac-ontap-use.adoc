...
summary: 'Examinez les considérations RBAC ONTAP pour ONTAP tools avant de l"utiliser dans un environnement de production.' 
---
= ONTAP RBAC dans ONTAP tools for VMware vSphere
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Les ONTAP tools for VMware vSphere prennent en charge le contrôle d'accès basé sur les rôles (RBAC) via ONTAP, ce qui vous permet de créer des utilisateurs et des rôles personnalisés qui restreignent l'accès aux ressources de stockage. Comprendre le processus de configuration et les niveaux de privilèges vous aide à mettre en place des contrôles d'accès appropriés avant de déployer ONTAP tools for VMware vSphere dans un environnement de production.
...
Les ONTAP tools for VMware vSphere incluent la prise en charge de la création d’un utilisateur ONTAP avec un rôle personnalisé.  Les définitions sont regroupées dans un fichier JSON que vous pouvez télécharger sur le cluster ONTAP .  Vous pouvez créer l'utilisateur et personnaliser le rôle en fonction de votre environnement et de vos besoins de sécurité.

La section suivante décrit brièvement les principales étapes de configuration. Consultez link:../configure/configure-user-role-and-privileges.html["Configurer les rôles et privilèges des utilisateurs ONTAP"] pour plus de détails.
...
. Faites défiler vers le bas jusqu'à *Utilisateurs et rôles* et sélectionnez `-->`.
...
Après avoir sélectionné les options de configuration de votre environnement, sélectionnez *Ajouter* et ONTAP crée l'utilisateur et le rôle. Le nom du rôle généré est la concaténation des valeurs suivantes :
...
Le nouvel utilisateur apparaît dans la liste sur la page *Utilisateurs et rôles*. Notez que les méthodes de connexion utilisateur HTTP et ONTAPI sont prises en charge.
