...
---
= Découvrez l'API REST des ONTAP tools
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
ONTAP tools for VMware vSphere 10 expose une adresse IP externe unique (anciennement équilibreur de charge) qui sert de passerelle vers les fonctionnalités de la machine virtuelle. Vous fournissez cette adresse lors de la configuration initiale. Cette adresse est utilisée par l’interface ONTAP tools Manager et permet d’accéder directement à la page de documentation Swagger et à l’API REST.
