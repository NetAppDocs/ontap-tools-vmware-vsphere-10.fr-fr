...
+
L'option de déploiement avancé utilise Trident en tant que fournisseur de stockage dynamique pour ONTAP pour créer des volumes. Le déploiement simplifié utilise le stockage local pour créer des volumes.
...
. Lorsque vous sélectionnez *OK*, les valeurs fournies seront à nouveau validées. Vous disposez de la disposition permettant de corriger les valeurs jusqu'à trois fois. Si vous ne parvenez pas à corriger au cours des 3 tentatives, l'installation du produit s'arrête et il est conseillé d'essayer l'installation sur une nouvelle machine virtuelle.
