...
* *Exigences de dimensionnement du système hôte* La mémoire recommandée pour chaque taille de déploiement est indiquée dans le tableau ci-dessous.  Pour les déploiements haute disponibilité (HA), vous avez besoin de trois fois la taille de l'appliance indiquée.
...
| 3300 et 6789-7300 | TCP | Plan de données inter-nœuds 
...
| Haute-disponibilité | Grand (L) | environ 100 000 | 256 [REMARQUE] Le nombre d'hôtes dans le tableau représente le total combiné de tous les vCenters connectés. 
|===
