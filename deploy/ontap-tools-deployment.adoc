...
NOTE: Pour permettre un déploiement haute disponibilité ultérieur, évitez de déployer la machine virtuelle ONTAP tools directement sur un hôte ESXi. Déployez-la plutôt au sein d'un cluster d'hôtes ESXi ou d'un pool de ressources.
...
. Sélectionnez *Actions* à droite de la page, sélectionnez *Importer un élément*, et importez le fichier OVA.
...
NOTE: Avant de procéder au déploiement, configurez le Distributed Resource Scheduler (DRS) du cluster sur Conservateur. Cela garantit que les machines virtuelles ne sont pas migrées pendant l'installation.

Les ONTAP tools for VMware vSphere sont initialement déployés en configuration non-HA. Pour passer à un déploiement HA, vous devrez activer l’ajout à chaud de CPU et le hot-plug de mémoire. Vous pouvez effectuer cette étape lors du processus de déploiement ou modifier les paramètres de la VM après le déploiement.
...
NOTE: Ne stockez jamais la machine virtuelle ONTAP tools for VMware vSphere sur les datastores vVols qu'elle gère.
...
** Si vous utilisez la version vCenter Server 7.0.3, suivez les étapes de la section *Et ensuite ?* à la fin du déploiement. image:../media/select-name.png["Sélectionnez un nom et un dossier"]
...
. Dans la fenêtre *Personnaliser le modèle*, remplissez les champs obligatoires. image:../media/custom-temp-p2.png["Personnaliser le modèle"]
+
...
. Lors de l'utilisation de la version vCenter Server 8.0.3, dans la fenêtre *Personnaliser le matériel*, activez les options *Ajout à chaud du processeur* et *Ajout à chaud de la mémoire* pour permettre la fonctionnalité HA. image:../media/customize-hw105-p2-1.png["Ajout à chaud du processeur"] image:../media/customize-hw105-p2.png["enfichage à chaud de la mémoire"]
...
.Et la suite ?
Si vous avez déployé ONTAP tools for VMware vSphere avec vCenter Server 7.0.3, suivez ces étapes après le déploiement.
