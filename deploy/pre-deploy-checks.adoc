...
| Adresse IP de l’application ONTAP tools. Il s’agit de l’adresse IP permettant d’accéder à l’interface web du gestionnaire ONTAP tools (load balancer) à `\https://<ip>:8443/virtualization/ui/` |  


| Adresse IP virtuelle ONTAP tools pour la communication interne. Cette adresse IP est utilisée pour la communication interne dans une configuration avec plusieurs instances ONTAP tools. Cette adresse IP ne doit pas être la même que l'adresse IP de l'application ONTAP tools. (Le plan de contrôle Kubernetes) |  
...
| Adresse IPv4 pour le nœud de gestion des outils ONTAP. Il s'agit d'une adresse IPv4 unique pour l'interface de gestion du nœud sur le réseau de gestion. Ceci est utilisé pour se connecter à l'appliance des outils ONTAP pour un accès de diagnostic à distance via SSH. |  
